...
func (r *teamRepo) Update(ctx context.Context, team *model.Team) error {
	query := `
		UPDATE teams SET name = $1, description = $2, university_id = $3, social_links = $4, avatar_url = $5 WHERE id = $6
	`
...
func (r *teamRepo) List(ctx context.Context) ([]*model.Team, error) {
	query := `
		SELECT t.id, t.name, t.description, t.university_id, t.social_links, t.avatar_url, u.name as university_name
        FROM teams t
        LEFT JOIN universities u ON t.university_id = u.id
    `
